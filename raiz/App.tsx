...
const App: React.FC = () => {
  ...
  if (!event) return <div className="h-screen flex items-center justify-center font-black animate-pulse">CARREGANDO SISTEMA...</div>;
      ...
      <div className="min-h-screen flex items-center justify-center p-6 bg-slate-950">
        <div className="bg-white p-8 rounded-[2.5rem] shadow-2xl w-full max-w-[380px] border-b-8 border-indigo-600">
          ...
          <form onSubmit={(e) => {
            ...
            {authMode === 'REGISTER' && (
              <input type="text" placeholder="Chave PIX (para saques)" value={registerPix} onChange={e => setRegisterPix(e.target.value)} className="w-full p-4 bg-slate-50 border-2 rounded-2xl font-bold" />
            )}
            ...
            <div className="h-full overflow-y-auto p-8 bg-slate-50 flex flex-col items-center">
              <div className="max-w-md w-full space-y-8 animate-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="bg-white p-6 rounded-3xl border border-slate-100 flex items-start gap-4 shadow-sm">
                  <div className="p-3 bg-amber-50 text-amber-500 rounded-2xl shrink-0"><Info size={24} /></div>
                  <div>
                    <h4 className="font-black text-slate-800 uppercase text-xs mb-1">Aviso Importante</h4>
                    <p className="text-[10px] text-slate-400 font-bold uppercase leading-relaxed">As séries compradas agora serão válidas para a próxima rodada caso esta já tenha começado.</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              {isAdminAuthenticated ? (
                ...
                <div className="max-w-md mx-auto bg-white p-12 rounded-[3.5rem] shadow-2xl border border-slate-100 mt-12 text-center">
                  ...
                  <div className="space-y-4 text-left">
                    <input type="text" placeholder="Usuário Admin" value={adminUserField} onChange={e => setAdminUserField(e.target.value)} className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-2xl font-bold focus:border-slate-900 transition-all" />
                    <input type="password" placeholder="Senha Operacional" value={adminPassField} onChange={e => setAdminPassField(e.target.value)} className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-2xl font-bold focus:border-slate-900 transition-all" />
                    <button onClick={() => {
