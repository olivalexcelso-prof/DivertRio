...
const processBallDraw = () => {
  ...
  // Marcar em todas as cartelas ativas no servidor
  ...
  if (winners.length > 0) {
    const winnerRecords: WinnerRecord[] = winners.map(w => {
      ...
      // Adiciona o prêmio à cartela no estado global
      if (!card.wonPrizes.includes(w.prize)) {
        card.wonPrizes.push(w.prize);
      }
    ...
    // Progressão automática de prêmios
...
io.on('connection', (socket) => {
  ...
  socket.on('registerUser', (userData: any) => {
    ...
    if (!user) {
      user = {
        ...
        balance: 100, // Saldo inicial para teste
        createdAt: Date.now(),
  ...
  // COMPRA CENTRALIZADA: Usa o preço definido pelo admin
  ...
  // AÇÕES DO ADMINISTRADOR (Sincronizam com todos)
...
httpServer.listen(PORT, () => console.log(`BINGO MASTER SERVER: Online na porta ${PORT}`));
